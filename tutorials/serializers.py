...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    email = serializers.EmailField(
        required=True,
        validators=[
            UniqueValidator(queryset=User.objects.all(), message="Email is already registered.")
        ]
    )
...
class UserPaymentGatewaySerializer(serializers.ModelSerializer):
    ...
    credentials = serializers.JSONField(required=True)  # required=True ensures it must be present

    class Meta:
        model = UserPaymentGateway
        fields = [
            'id', 'user_id', 'payment_gateway_id', 'payment_gateway_name',
            'credentials', 'status', 'is_live_mode',
    ...
    def validate_credentials(self, value):
        ...
        for key in required_keys:
            if key not in value:
                raise serializers.ValidationError(f"'{key}' is required in credentials.")
        return value